...
export default function DashboardPage() {
  ...
  const groupId = typeof window !== "undefined" ? localStorage.getItem("polla_group") : null;
  const [matches, setMatches] = useState<Match[]>([]);
  const [predictions, setPredictions] = useState<Prediction[]>([]);
  const [otherPredictionsMap, setOtherPredictionsMap] = useState<Record<number, OtherPrediction[]>>({});
    ...
    const loadData = async () => {
      try {
        ...
        const [matchesRes, predictionsRes] = await Promise.all([
          supabase.from("matches").select("*").order("fecha_partido", { ascending: true }),
        ...
        if (matchesRes.data && matchesRes.data.length > 0 && memberIds.length > 0) {
          const matchIds = matchesRes.data.map((m) => m.id);
          const { data: allPredictions } = await supabase
            .from("predictions")
            .select("user_id, match_id, pred_goles_local, pred_goles_visitante, puntos_ganados, users(nombre, avatar_url)")
          ...
          if (allPredictions) {
            ...
            for (const pred of allPredictions) {
              const users = pred.users as unknown as { nombre: string; avatar_url: string | null } | null;
              if (!users) continue;
  ...
  const getPrediction = useCallback(
    (matchId: number) => predictions.find((p) => p.match_id === matchId) ?? null,
    [predictions]
  );
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        >
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {groupName ? `Grupo: ${groupName} · ` : ""}Realiza tus pronosticos para cada fase del Mundial
          </p>
        </motion.div>

        <div className="flex gap-2 overflow-x-auto pb-2 mb-6 scrollbar-hide">
          {PHASES.map((phase) => {
            const count = matches.filter((m) => m.fase === phase).length;
            return (
              <button
                key={phase}
